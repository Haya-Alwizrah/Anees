...
الشروط:
- لا تنسخ القصص المرجعية حرفيًا،أنشئ قصة جديدة ومبتكرة.
...
- تأكد أن القصة سليمة نحوياً (مثل: مؤنث أو مذكر + فرد، اثنان، أو جمع)
- استخدم اسماء عربية واضحة لا تشتت الطفل (لا تستخدم اسم فهد او اي اسم اخر قد يسئ الطفل فهمه على انه هو نوع الحيوان)
- يجب ان تكون احداث القصة منطقية وواقعية.
"""
...
- انشئ 3 أجوبة خاطئة قريبة من الإجابة الصحيحة 
- لا تجعل الاجوبة سهلة جداً يجب على الطفل التفكير وتذكر القصة لأختيار الجواب الصحيح
- استخدم لغة عربية واضحة وبسيطة.
...
"""
        ...
        return response.content
#---------------------------------------------------------------------------------------------------------------------------------------------------------
    def explain_words(self, story):
        prompt = f"""
أنت معلم لغة عربية للأطفال.

اقرأ القصة التالية وحدد الكلمات التي قد تكون صعبة أو غير مألوفة لطفل يتعلم اللغة العربية.

القصة:
{story}

الشروط:
- اختر الكلمات التي قد يحتاج الطفل إلى معرفة معناها.
- لا تختر الكلمات العربية البسيطة والشائعة جدًا.
- اختر فقط الكلمات الموجودة فعليًا في القصة.
- لا تكرر الكلمات.
- اشرح معنى كل كلمة بطريقة بسيطة جدًا ومناسبة للأطفال.
- يجب أن يكون المعنى مناسبًا لسياق الكلمة داخل القصة.
- لا تضف أي معلومات غير ضرورية.
- لا تشرح علامات الترقيم.
...
{{
    "words": [
        {{
            "word": "الكلمة",
            "meaning": "معنى الكلمة بطريقة بسيطة للطفل"
        }}
    ]
}}
"""
